...
public class AuthController {
    ...
    public String verifyEmail(@RequestParam("token") String token, HttpSession session) {
        ...
        if(user!=null){
            ...
            Message.builder().type(MessageType.green)
            .content("Email in verified, Login Now")
            .build());
                return "success_page";
            }

            session.setAttribute("message", 
            Message.builder().type(MessageType.red)
            .content("Email in not verified, Token is not associated with user")
            .build());
            return "error_page";
        }
